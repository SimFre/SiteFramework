...
class adLDAP {
    ...
    */
    public function connect() {
        ...
        $port = 0;
        if (is_null($this->_ldap_port) && !$this->_use_ssl) { $port = 389; }
        elseif (is_null($this->_ldap_port) && $this->_use_ssl) { $port = 636; }
        else { $port = $this->_ldap_port; }
        
        if ($this->_use_ssl){
            $this->_conn = ldap_connect("ldaps://".$dc, $port);
        } else {
    ...
    */
    public function user_info($username,$fields=NULL,$isGUID=false){
        ...
        if ($fields===NULL){ $fields=array("userprincipalname", "samaccountname","mail","memberof","department","displayname","telephonenumber","primarygroupid","objectsid"); }
